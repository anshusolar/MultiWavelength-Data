...
output_dir = 'C:/Users/knadi/Desktop/euvi_284_frames'
os.makedirs(output_dir, exist_ok=True)

# Load EUVI 284 data
st_files_284_A = glob.glob('C:/Users/knadi/OneDrive/Desktop/USO/Stereo_data/stereo_20250526023633_101741/secchi/L0/a/img/euvi/20240529/*.fts')
st_files_284_A.sort()
print(f"Number of EUVI 284 files: {len(st_files_284_A)}")

# Process each file and save images
for i, file in enumerate(st_files_284_A):
    try:
        map_euvi = sunpy.map.Map(file)
        print(f"Processing EUVI map at {map_euvi.date}")
        
        # Apply limb enhancement
        enhanced_map = limb_enhance(map_euvi)
        smoothed_data = uniform_filter(enhanced_map.data, 7) / map_euvi.exposure_time.value
        plot_map = sunpy.map.Map(smoothed_data, map_euvi.meta)
        ...
        #fig.suptitle(f'EUVI-A 284 $\\mathrm{{Å}}$ {plot_map.date}', fontsize=13)
        
        # Save the figure
        output_file = os.path.join(output_dir, f'euvi_{i:03d}zoom.png')
